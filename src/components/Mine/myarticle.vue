<template>
    ...
    <ul>
        <li>{{pullDownMsg}}</li>
          <li v-for="item in articleLis" :key="item.Id">
...
mounted(){
      this.axios.get('/api/stragies/getmyarticle').then( (res)=>{
        this.Msg = res.data.status;
        this.articleLis = res.data.data;
        this.$nextTick( ()=>{
            setTimeout(() => {
...
methods:
{
    ...
    handleTouchEnd:function(pos){
        if(pos.y > 15 ){
            this.pullDownMsg="";
            this.PageIndex++;
            this.axios.get('/api/stragies/getmyarticle?page='+this.PageIndex+'&rows='+this.PageSize).then( (res)=>{
