        ...
        <li>{{pullDownMsg}}</li>
          <li v-for="item in stragiesList" :key="item.Id">
            ...
            <div class="info_list">
                <h2>{{item.Title}}</h2>
                <i>{{item.eTitle}}</i>
                <div class="Line"><span class="p-left">观众评<span class="grade">{{item.grade}}</span></span><span class="p-right">作者:{{item.author}}</span></div>
...
mounted(){
      this.axios.get('/api/stragies/GetTopStragies?type=2').then( (res)=>{
...
methods:
{
    ...
    {
    this.$router.push('/GameStragies/Detail/2/'+id);
    },
    ...
    handleTouchEnd:function(pos){
        if(pos.y > 15 ){
            this.pullDownMsg="";
            this.PageIndex++;
            this.axios.get('/api/stragies/GetStragies?type=2&page='+this.PageIndex+'&rows='+this.PageSize).then( (res)=>{
