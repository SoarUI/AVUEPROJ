...
mounted(){
      this.axios.get('/api/stragies/GetTopStragies?type=1').then( (res)=>{
...
methods:
{
    ...
    {
    this.$router.push('/GameStragies/Detail/1/'+id);
    },
    ...
    handleTouchEnd:function(pos){
        if(pos.y > 15 ){
            this.pullDownMsg="";
            this.PageIndex++;
            this.axios.get('/api/stragies/GetStragies?type=1&page='+this.PageIndex+'&rows='+this.PageSize).then( (res)=>{
