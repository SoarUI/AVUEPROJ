...
mounted(){
    this.axios.get('/api/game/GetTopNewGames?type=3').then( (res)=>{
...
methods:{
    ...
    {
    this.$router.push('/NewGames/Detail/3/'+id);
    },
    ...
    handleTouchEnd:function(pos){
        if(pos.y > 15 ){
            this.pullDownMsg="";
            this.PageIndex++;
            this.axios.get('/api/game/GetTopNewGames?type=3&page='+this.PageIndex+'&rows='+this.PageSize).then( (res)=>{
